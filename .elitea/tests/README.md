...
This directory contains the AI-native test framework for automated testing of ELITEA SDK toolkits and agents. It works together with agent definitions and toolkit configs stored under `.elitea/`.
...
path: .elitea\tool_configs\git-config.json
...
The config section specifies the toolkit configuration file to use. Configs are stored under `.elitea/tool_configs/`.
...
```bash
elitea agent execute-test-cases \
  .elitea/agents/test-runner.agent.md \
  --test-cases-dir .elitea/tests/testcases \
  --results-dir .elitea/tests/results \
  --data-generator .elitea/agents/test-data-generator.agent.md \
  --dir .
```
...
elitea agent execute-test-cases \
  .elitea/agents/test-runner.agent.md \
  --test-cases-dir .elitea/tests/testcases/github \
  --results-dir .elitea/tests/results \
  --data-generator .elitea/agents/test-data-generator.agent.md \
  --dir .
...
elitea agent execute-test-cases \
  .elitea/agents/test-runner.agent.md \
  --test-cases-dir .elitea/tests/testcases/confluence \
  --results-dir .elitea/tests/results \
  --data-generator .elitea/agents/test-data-generator.agent.md \
  --dir .
...
elitea agent execute-test-cases \
  .elitea/agents/test-runner.agent.md \
  --test-cases-dir .elitea/tests/testcases/ado \
  --results-dir .elitea/tests/results \
  --data-generator .elitea/agents/test-data-generator.agent.md \
  --dir .
...
elitea agent execute-test-cases \
  .elitea/agents/test-runner.agent.md \
  --test-cases-dir .elitea/tests/testcases/jira \
  --results-dir .elitea/tests/results \
  --data-generator .elitea/agents/test-data-generator.agent.md \
  --dir .
```
...
elitea toolkit test github \
  --tool list_branches_in_repo \
  --config .elitea/tool_configs/git-config.json

# Test with parameters
elitea toolkit test confluence \
  --tool get_page_tree \
  --config .elitea/tool_configs/confluence-config.json \
...
```json
{
  ...
  "config_path": ".elitea\\tool_configs\\git-config.json",
  "passed": true,
  "steps": [
    {
      "step_number": 1,
...
1. **Choose the appropriate toolkit directory**: Create test case in `.elitea/tests/testcases/<toolkit_name>/` (e.g., `github`, `ado`, `confluence`, `jira`)
2. **Create test case file**: Name it `TC-XXX_description.md` following the numbering convention
3. **Create or reuse config file**: Place toolkit config in `.elitea/tool_configs/`
...
- **GitHub** (`.elitea/tests/testcases/github/`) - GitHub API operations, branches, PRs, issues, files
- **Azure DevOps** (`.elitea/tests/testcases/ado/`) - ADO repositories, branches, PRs, files
- **Confluence** (`.elitea/tests/testcases/confluence/`) - Confluence pages, search, attachments
- **Jira** (`.elitea/tests/testcases/jira/`) - Jira issues, projects, workflows
## Validation Rules
...
- When a PR changes files in elitea_sdk/tools/github/, only GitHub tests run
- When a PR changes files in elitea_sdk/tools/confluence/, only Confluence tests run
- When a PR changes files in elitea_sdk/tools/jira/, only Jira tests run
...
- Use the correct path format: `.elitea/tool_configs/<config-file>.json`
- Verify the config file exists under `.elitea/tool_configs/`
