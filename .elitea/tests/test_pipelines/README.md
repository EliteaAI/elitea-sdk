# EliteA SDK Test Pipelines Framework

**Declarative end-to-end testing framework for EliteA SDK toolkits and agents**

## Overview

A comprehensive, production-grade testing system that validates EliteA SDK components through declarative YAML pipeline definitions. Designed for CI/CD integration with automated setup, execution, RCA on failures, and detailed reporting.
...
| **Dual Execution Modes** | Run locally (SDK-only) or remotely (EliteA Platform) |
...
**Tech Stack**: LangGraph (pipelines), EliteAClient (LLMs), YAML configs, Python logging, HTML reports
...
**Tech**: Direct toolkit imports, in-memory LangGraph, EliteAClient for LLMs
...
export DOCKER_COMMAND=".elitea/tests/test_pipelines/run_test.sh --local suites/ado ADO01"
...
setup:
  - name: Setup Configuration
    ...
    config:
      ...
      elitea_title: github-creds      # Secret name on platform
      data:
        token: ${GITHUB_TOKEN}
...
for name in ['elitea_sdk', 'utils_local', 'run_suite', 'setup', 'seed_pipelines', 'cleanup']:
...
**Docker**: `ghcr.io/eliteaai/elitea-sdk:pyodide` with git config fix, dynamic commands via `$DOCKER_COMMAND`
...
**Framework Version**: 2.0 | **Last Updated**: Feb 2026 | **Maintainer**: EliteA SDK Team
