...
def run_suite_local(
    ...
    runner = IsolatedPipelineTestRunner(
        ...
        verbose=logger.verbose if logger else False,
        sdk_log_level=sdk_log_level,  # Control elitea_sdk logging verbosity
...
def main():
    ...
    parser.add_argument("--local", nargs='?', const=True, default=None,
                        help="Local mode: run pipelines without backend. "
                             "Optional: set elitea_sdk log level (debug|info|warning|error, default: error)")
    ...
    launch_name = f"EliteA SDK - {suite_name}"
