...
def find_configuration_by_title(ctx: SetupContext, elitea_title: str, type_filter: str = None) -> Optional[dict]:
...
def create_configuration(ctx: SetupContext, config_type: str, label: str, elitea_title: str, data: dict) -> dict:
...
def ensure_configuration(ctx: SetupContext, config_type: str, elitea_title: str, data: dict) -> dict:
    """
    ...
    """
    existing = find_configuration_by_title(ctx, elitea_title, config_type)

    if ctx.dry_run:
        if existing:
            ctx.log(f"[DRY RUN] Would update configuration: {elitea_title}", "info")
        else:
            ctx.log(f"[DRY RUN] Would create configuration: {elitea_title}", "info")
    ...
    if existing:
        ...
        if not update_data:
            ctx.log(f"Configuration '{elitea_title}' exists, no valid updates to apply", "info")
            return {"success": True, "skipped": True, "reason": "no valid update data"}

        ctx.log(f"Configuration '{elitea_title}' exists (ID: {existing['id']}), updating fields: {list(update_data.keys())}", "info")
        ...
        ctx.log(f"Creating configuration '{elitea_title}'...", "info")
        return create_configuration(ctx, config_type, elitea_title.title(), elitea_title, data)
...
def handle_configuration(step: dict, ctx: SetupContext) -> dict:
    ...
    Config should contain:
      ...
      - elitea_title: The configuration title/name
    ...
    return ensure_configuration(ctx, config_type, elitea_title, data)
...
def write_env_file(env_vars: dict, output_path: Path):
    ...
    Existing lines are preserved (the target may be a user-maintained .env passed
    via --env-file); keys already present are updated in place, new keys appended.
    """
    ...
    if pending:
        lines.append(f"\n# Generated by setup.py at {datetime.now().isoformat()}")
