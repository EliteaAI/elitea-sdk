...
Uses EliteAClient for LLM access (same as run_streamlit.py).

This module provides:
- IsolatedPipelineTestRunner: Executes pipeline tests locally
- create_elitea_client(): Creates EliteAClient from environment
...
Environment variables (from .env):
    DEPLOYMENT_URL      EliteA deployment URL
    API_KEY             EliteA API key
    PROJECT_ID          EliteA project ID
"""
...
def configure_elitea_sdk_logging(log_level: str = 'error'):
    """
    Configure elitea_sdk package logging level.

    By default, suppress elitea_sdk.* loggers to keep test output clean.

    Why this is needed:
    - Local test execution instantiates full toolkit stack locally
    - This triggers elitea_sdk initialization logs:
      * elitea_sdk.configurations
      * elitea_sdk.tools
      * elitea_sdk.runtime.langchain.langraph_agent
      * etc.
    ...
    Args:
        log_level: Log level for elitea_sdk loggers
    ...
    elitea_loggers = [
        ...
        'elitea_sdk.configurations',
        'elitea_sdk.cli',
        'elitea_sdk.community',
    ]

    for logger_name in elitea_loggers:
...
def configure_file_logging(log_file: str, sdk_log_level: str = 'error', verbose: bool = False):
    ...
    Args:
        ...
        sdk_log_level: Log level for elitea_sdk loggers ('debug', 'info', 'warning', 'error')
    ...
    configure_elitea_sdk_logging(sdk_log_level)

    # Override elitea_sdk loggers to DEBUG for file logging
    # This allows DEBUG logs to be emitted and captured by the file handler
    # Must happen AFTER configure_elitea_sdk_logging() which would reset them
    ...
    logger.debug(f"elitea_sdk loggers set to DEBUG: {', '.join(elitea_loggers)}")
    logger.debug(f"elitea_sdk logger level: {logging.getLogger('elitea_sdk').level} (should be {logging.DEBUG})")
...
def create_elitea_client():
    """
    Create EliteAClient from environment variables.
    ...
    logger.info(f"Creating EliteAClient for {base_url} (project: {project_id})")
...
class IsolatedPipelineTestRunner:
    ...
    Uses EliteAClient for LLM access (same as streamlit).
    ...
    def __init__(
        ...
        Args:
            ...
            elitea_client: Optional pre-created EliteAClient
            llm: Optional pre-created LLM instance
            sdk_log_level: Log level for elitea_sdk loggers
        ...
        # Do NOT call configure_elitea_sdk_logging() here, as it would reset logger levels
    ...
    def _ensure_client(self):
        """Ensure EliteAClient is initialized."""
        if self.elitea_client is None:
            self.elitea_client = create_elitea_client()
        return self.elitea_client

    def _get_llm(self, model: str = 'gpt-5-mini', temperature: float = 0.0, max_tokens: int = 4096):
        """
        Get LLM from EliteAClient or return pre-created instance.

        If LLM was passed during initialization, returns that.
        Otherwise, creates one using EliteAClient.
    ...
    def run_test(
        ...
        from elitea_sdk.runtime.middleware import ToolExceptionHandlerMiddleware, TransformErrorStrategy
