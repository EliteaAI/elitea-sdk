...
Scripts to copy or delete apps, configs, toolkits, and MCPs between EliteA environments (dev, stage, next). Run from this directory so `elitea_api_common` is on the path.
...
1. **copy_configs.py** — copy configurations (by `elitea_title`)
