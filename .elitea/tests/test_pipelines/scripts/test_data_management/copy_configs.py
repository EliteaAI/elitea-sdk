#!/usr/bin/env python3
"""
Copy configurations from source EliteA environment to target.

Steps:
  1. List all configurations from the source project
  2. List all configurations from the target project
  3. Compare by elitea_title — identify configs that exist on source but not target
...
from elitea_api_common import ENV_URLS, EliteAAPI
...
def build_config_payload(config: dict) -> dict:
    ...
    elitea_title, label, type, shared, data.
    """
    return {
...
def main() -> int:
    ...
    source = EliteAAPI(ENV_URLS[args.source_env], args.source_project_id, args.source_token)
    target = EliteAAPI(ENV_URLS[args.target_env], args.target_project_id, args.target_token)
    ...
    # Step 3: Compare by elitea_title — find configs missing on target
    # Skip types listed in IGNORED_CONFIG_TYPES (managed separately per environment)
    target_titles: set[str] = {c["elitea_title"] for c in target_configs}
    to_copy = [
        c for c in source_configs
        if c["elitea_title"] not in target_titles and c.get("type") not in IGNORED_CONFIG_TYPES
    ]

    log.info("Configs to copy: %d (missing on target)", len(to_copy))
    for c in to_copy:
        log.info("  - %s (type=%s, label=%s)", c["elitea_title"], c["type"], c.get("label", ""))
