#!/usr/bin/env python3
"""
ReportPortal Reporter for EliteA SDK Test Framework
...
class ReportPortalReporter:
        ...
        if config.enabled:
            try:
                ...
                rp_config = RPConfig(
                    ...
                    launch_name=config.launch_name or "EliteA SDK Tests"
                )
    ...
    def launch(self, name: Optional[str] = None, description: str = "", 
        ...
        try:
            # Start the launch
            launch_name = name or self.config.launch_name or "EliteA SDK Tests"
            attrs = attributes or self.config.launch_attributes or [
                {"key": "framework", "value": "elitea-sdk"},
