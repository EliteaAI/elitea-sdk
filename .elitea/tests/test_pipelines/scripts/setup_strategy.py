...
class LocalSetupStrategy(SetupStrategy):
    ...
    def __init__(self, toolkit_configs: Optional[Dict[str, Any]] = None, llm: Optional[Any] = None):
        ...
        self._elitea_client = None
        self._llm = llm  # Use provided LLM or create on demand
        self._configuration_data: Dict[str, Dict[str, Any]] = {}  # Store configuration data by elitea_title
    ...
    def _create_toolkit_tools(self, toolkit_info: Dict[str, Any], ctx: "SetupContext") -> List[Any]:
        ...
        Uses get_tools from elitea_sdk.runtime.toolkits.tools for consistent
        ...
        # Initialize elitea client if needed
        if self._elitea_client is None:
            logger.info("[TOOLKIT CREATE] Creating EliteAClient")
            self._elitea_client = self._create_elitea_client()
            logger.info("[TOOLKIT CREATE] EliteAClient created")
        ...
        try:
            ...
            logger.info("[TOOLKIT CREATE] Calling get_tools from elitea_sdk.runtime.toolkits.tools")
            tools = get_tools(
                tools_list=[tool_config],
                elitea_client=self._elitea_client,
                llm=self._llm,
                memory_store=None,
                debug_mode=False,
            )
    ...
    def _build_toolkit_settings(self, toolkit_type: str, config: Dict[str, Any], load_from_env) -> Dict[str, Any]:
        ...
        2. If elitea_title is present, merge in stored configuration data from configuration step
    ...
    def _create_elitea_client(self) -> Optional[Any]:
        """Create EliteAClient for toolkit initialization."""
    ...
    def _create_llm(self) -> Optional[Any]:
        ...
        try:
            llm = self._elitea_client.get_llm(
    ...
    def handle_configuration(
        ...
        if not elitea_title:
            ctx.log("[LOCAL] No elitea_title provided", "warning")
            return {"success": True, "skipped": True, "reason": "no elitea_title", "local": True}
        
        # Store locally - credentials come from env
        self.created_configurations[elitea_title] = {
            "type": config_type,
            "title": elitea_title,
            "data": data,
        }
        
        # Also store the data for lookup by elitea_title
        self._configuration_data[elitea_title] = data
