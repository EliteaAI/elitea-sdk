...
Comprehensive test suite for GitLab toolkit under `elitea_sdk/tools/gitlab/`.
...
# RCA (Optional)
SDK_REPO=EliteaAI/elitea-sdk             # SDK repo for RCA analysis
...
### Run All Tests
```bash
bash .elitea/tests/test_pipelines/run_all_suites.sh suites/gitlab
```

### Run Specific Test
```bash
bash .elitea/tests/test_pipelines/run_test.sh suites/gitlab GL01
```

### Local Mode (No Backend)
```bash
bash .elitea/tests/test_pipelines/run_all_suites.sh --local suites/gitlab
```

### Full Workflow (Setup + Seed + Run + Cleanup)
```bash
bash .elitea/tests/test_pipelines/run_test.sh --all suites/gitlab GL01
```
...
- Consult GitLab toolkit documentation in `elitea_sdk/tools/gitlab/`
