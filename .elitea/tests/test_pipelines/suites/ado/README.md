...
This directory contains a comprehensive test suite for validating Azure DevOps toolkit functionality through the EliteA SDK, covering Repos, Wiki, and Work Items components.
...
Create a [.env](.elitea/tests/test_pipelines/.env) file or set the following environment variables:
...
Located at `.elitea/tests/test_pipelines/configs/ado-config.json`:
