# EliteA SDK - Visual Architecture Diagrams
...
graph TB
    ...
    subgraph ENTRY["SDK Entry Point"]
        CLIENT[EliteAClient<br/>runtime/clients/client.py<br/>Main SDK Interface]
    end
    ...
    subgraph TKITS["Toolkit System"]
        ...
        subgraph PLATFORM["Platform Toolkits<br/>(Use EliteAClient APIs)"]
            MCP[MCP Toolkit]
...
- **EliteAClient** is the MAIN SDK ENTRY POINT - called by platform backend/UI
- **Platform Toolkits** (MCP, Application, Datasource, Artifact, Prompt) use EliteAClient APIs
...
graph BT
    ...
    subgraph L1["Layer 1: Core Runtime - CRITICAL IMPACT"]
        CLIENT[EliteAClient<br/>SDK Entry Point]
        ASST[Assistant]
...
sequenceDiagram
    ...
    participant Client as EliteAClient
...
flowchart TD
    ...
    Q1 -->|EliteAClient<br/>SDK Entry Point| CRIT0[🔴 CRITICAL<br/>Platform APIs]
...
graph TB
    subgraph "Agent Execution Flow"
        direction LR
        A1[Backend Request] --> A2[EliteAClient]
        A2 --> A3[Assistant]
        A3 --> A4[LangGraph]
        A4 --> A5[Nodes]
        A5 --> A6[Tools]
    ...
    subgraph "Pipeline Execution Flow"
        direction LR
        P1[Backend Trigger] --> P2[EliteAClient]
        P2 --> P3[Assistant]
        P3 --> P4[LangGraph]
        P4 --> P5[Nodes]
...
| EliteAClient | Low | Critical | 🔴 High | P0 - Platform APIs |
