...
- `automation_bug` — tool returned a processed error (contains `SupportEliteA@epam.com`) or unexpected HTTP status code, failure reproduced on rerun. Requires TA team verification. Do NOT modify test.
...
| 2a | Any `tool_output` contains `"SupportEliteA@epam.com"` OR `tool_output` contains an error with unexpected HTTP status code (401, 403, 500, 502, 503) AND test does not explicitly assert on status codes | **automation_bug (pending rerun)** | Tool returned processed error or unexpected status code; rerun to confirm — if reproduced → `blocked[]` with `blocker_type: "automation_bug"`, if passes → `flaky` |
| 2b | Any `tool_output` contains raw `Traceback`, `ToolException`, or unprocessed exception text WITHOUT `"SupportEliteA@epam.com"` | **sdk_bug** | SDK returned unprocessed exception; error handling is broken |
...
- `tool_output` contains raw `Traceback`, `ToolException`, or unprocessed exception WITHOUT `"SupportEliteA@epam.com"` → SDK failed to handle the error gracefully — classify as `sdk_bug`.
...
Is success=false AND output=null?
  ...
  └─ NO  → Does any tool_output contain "SupportEliteA@epam.com" OR unexpected HTTP status code (401/403/500/502/503) with test not asserting on status codes?
            └─ YES → automation_bug (pending rerun) (rule 2a)
            └─ NO  → Does tool_output contain raw Traceback/ToolException without "SupportEliteA@epam.com"?
                      └─ YES → sdk_bug (rule 2b)
...
| Environment | Command |
|------------|---------|
| **local** | `bash -c ".elitea/tests/test_pipelines/run_test.sh --local --setup --timeout 180 suites/<suite> <tests>"` |
| **dev/stage/prod** | `bash -c ".elitea/tests/test_pipelines/run_test.sh --all --timeout 180 suites/<suite> <tests>"` |
...
1. **Errors-only file (preferred):** `filesystem_read_file(path=".elitea/tests/test_pipelines/test_results/suites/<suite>/results_errors_only.json")` — pre-filtered to failing tests only, smaller, faster to parse.
2. **Full file (fallback):** `filesystem_read_file(path=".elitea/tests/test_pipelines/test_results/suites/<suite>/results.json")` — filter to `test_passed != true` entries. For large files, use `filesystem_read_file_chunk` with line ranges.
...
**D. Read PR change context (ONE TIME, optional) using `filesystem_read_file`.** Path: `.elitea/tests/test_pipelines/pr_change_context.json`.
...
**A. Read framework docs ONCE using `filesystem_read_file` or `filesystem_read_file_chunk`:** `.elitea/tests/test_pipelines/README.md` — focus on test YAML structure, node types, `continue_on_error`, `structured_output`, error handling.
...
**SDK bugs** → classify as `blocked[]`. Search `elitea_sdk/tools/<toolkit>/` for code locations.
...
- Tool output contains raw exception/traceback without processed error message (no `"SupportEliteA@epam.com"` contact suggestion) — SDK failed to handle the error gracefully
...
Max 3 attempts per test. Location: `.elitea/tests/test_pipelines/suites/<suite>/tests/*.yaml`
...
- **Read test YAML:** `filesystem_read_file(path=".elitea/tests/test_pipelines/suites/<suite>/tests/<test>.yaml")`
...
7. **Modifying test validation to accept processed tool errors** — if tool output contains a processed error with `"SupportEliteA@epam.com"` or an unexpected HTTP status code, the test is correct; the environment needs verification by TA team
...
- proper error message will always contain "SupportEliteA@epam.com"` contact suggestion
...
Key files: `run_pipeline.py`, `run_suite.py`, `setup.py`/`cleanup.py`, `utils_common.py`/`utils_local.py`, `pattern_matcher.py`, `logger.py` in `.elitea/tests/test_pipelines/scripts/`. Read using `filesystem_read_file`. Prefer YAML fixes 95% of the time.
...
Write using `filesystem_write_file` to `.elitea/tests/test_pipelines/test_results/suites/<suite>/fix_output.json` — valid JSON only, no markdown fences.
...
```json
{
  ...
  "blocked": [{
    ...
    "sdk_component": "elitea_sdk/tools/xray/api_wrapper.py",
    ...
    "raw_error_snippet": "Tool execution error! 401 Client Error: Unauthorized for url: https://... If you need assistance, contact SupportEliteA@epam.com",
  ...
  "pr_regressions": [{
    ...
    "sdk_component": "elitea_sdk/tools/github/api_wrapper.py",
