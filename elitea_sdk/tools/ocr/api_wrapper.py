...
class OCRApiWrapper(BaseToolApiWrapper):
    ...
    def validate_settings(self):
        ...
        # self.artifact = self.elitea.artifact(self.artifacts_folder)
    ...
    def _process_images_with_llm(self, images: List[str] | str, prompt: Optional[str] = None) -> Any:
        ...
        try:
            ...
            for img_path in images:
                # Download the image
                image_data = self.elitea.download_artifact(self.artifacts_folder, img_path)
    ...
    def _process_with_tesseract(self, image_path: str) -> str:
        ...
        try:
            ...
            data = self.elitea.download_artifact(self.artifacts_folder, image_path)
    ...
    def recognize(self, file_path: str, prompt: Optional[str] = None, prepare_text: Optional[bool] = False) -> Dict[str, Any]:
        ...
        file_exists = False
        for f in self.elitea.list_artifacts(self.artifacts_folder).get('rows', []):
            if f['name'] == file_path:
                file_exists = True
                break
        ...
        for img_path in result['images']:
            try:
                self.elitea.delete_artifact(self.artifacts_folder, img_path)
                if img_path.endswith("_enhanced.png"):
                    ...
                    self.elitea.delete_artifact(self.artifacts_folder, original_path)
        ...
        if "original_file_type" in result and result["original_file_type"] == "office":
            try:
                self.elitea.delete_artifact(self.artifacts_folder, file_path)
    ...
    def prepare_text(self, image_path: str) -> str:
        ...
        try:
            # Download the image
            image_data = self.elitea.download_artifact(self.artifacts_folder, image_path)
            ...
            self.elitea.create_artifact(self.artifacts_folder, processed_filename, img_bytes.getvalue())
    ...
    def pdf_to_images(self, pdf_path: str) -> List[str]:
        ...
        try:
            # Get PDF data
            pdf_data = self.elitea.download_artifact(self.artifacts_folder, pdf_path)
            ...
            for page_num, page in enumerate(doc):
                ...
                if classification['type'] == 'multiple_photos' and len(classification['regions']) > 1:
                    ...
                    for i, (x, y, w, h) in enumerate(classification['regions']):
                        ...
                        img_bytes.seek(0)
                        self.elitea.create_artifact(self.artifacts_folder, region_filename, img_bytes.read())
                    ...
                    region_pil.save(img_bytes, format="PNG")
                    img_bytes.seek(0)
                    self.elitea.create_artifact(self.artifacts_folder, image_filename, img_bytes.read())
    ...
    def office_to_pdf(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                office_data = self.elitea.download_artifact(self.artifacts_folder, file_path)
                ...
                if os.path.exists(pdf_path):
                    ...
                    self.elitea.create_artifact(self.artifacts_folder, pdf_filename, pdf_data)
