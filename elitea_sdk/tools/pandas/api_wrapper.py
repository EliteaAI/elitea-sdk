...
class PandasWrapper(BaseToolApiWrapper):
    ...
    def bytes_content(self, content: Any, filename: str) -> bytes:
        ...
        """
        if not content:
            content = self.elitea.download_artifact(self.bucket_name, filename)
    ...
    def _get_dataframe(self, filename: str) -> pd.DataFrame | None:
        ...
        artifacts = self.elitea.list_artifacts(self.bucket_name)
        df_exists = False
        ...
        if df_exists:
            try:
                _df = self.elitea.download_artifact(self.bucket_name, df_name)
        ...
        try:
            ...
            file_content = self.elitea.download_artifact(self.bucket_name, filename)
    ...
    def _save_dataframe(self, df: pd.DataFrame, filename: str) -> None:
        ...
        respone = self.elitea.create_artifact(self.bucket_name, df_name, bytes_io.getvalue())
        return respone    
        ...
        if result.get('chart'):
            ...
            if isinstance(result['chart'], list):
                for ind, chart in enumerate(result['chart']):
                    ...
                    self.elitea.create_artifact(self.bucket_name, chart_filename, chart_data)
                    chart_url = f"{self.elitea.base_url}/api/v1/artifacts/artifact/default/{self.elitea.project_id}/{self.bucket_name}/{chart_filename}"
    ...
    def save_dataframe(self, source_df: str, target_file: str) -> str:
        ...
        response = self.elitea.create_artifact(
            self.bucket_name,
            target_file,
            content
        )
