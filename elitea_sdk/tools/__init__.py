...
def _inject_toolkit_id(tool_conf: dict, toolkit_tools) -> None:
    ...
    if isinstance(toolkit_id, int):
        ...
        # Standalone and CLI runs have no backend toolkit row, so an absent `id` is
        # expected there and nothing downstream can act on it: injection is skipped
...
def _patch_tool_invoke(tool) -> None:
    ...
    that reaches ``EliteACallback.on_tool_start`` as ``kwargs["metadata"]`` is the LangGraph
...
def _inject_display_metadata(tool_conf: dict, toolkit_tools: list) -> None:
    ...
    ``display_name`` carries the user-friendly ``EliteATool.name`` (e.g. "My Jira Project")
    ...
    forwarded into the LangGraph run-config and becomes visible in ``EliteACallback``.
...
def _safe_import_tool(tool_name, module_path, get_tools_name=None, toolkit_class_name=None):
    ...
    try:
        module = __import__(f'elitea_sdk.tools.{module_path}', fromlist=[''])
...
_safe_import_tool('github', 'github', 'get_tools', 'EliteAGitHubToolkit')
_safe_import_tool('openapi', 'openapi', 'get_tools', 'EliteAOpenAPIToolkit')
...
_safe_import_tool('gitlab', 'gitlab', 'get_tools', 'EliteAGitlabToolkit')
_safe_import_tool('gitlab_org', 'gitlab_org', 'get_tools', 'EliteAGitlabSpaceToolkit')
...
_safe_import_tool('bitbucket', 'bitbucket', 'get_tools', 'EliteABitbucketToolkit')
...
_safe_import_tool('aha', 'aha', 'get_tools', 'AhaToolkit')
...
_safe_import_tool('yagmail', 'yagmail', 'get_tools', 'EliteAYagmailToolkit')
...
_safe_import_tool('localgit', 'localgit', None, 'EliteALocalGitToolkit')
...
try:
    from elitea_sdk import community  # noqa: F401
...
def get_tools(tools_list, elitea, llm, store: Optional[BaseStore] = None, *args, **kwargs):
    tools = []
