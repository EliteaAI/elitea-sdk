...
def get_file_bytes_from_artifact(elitea, filepath: str) -> tuple:
    """Get file bytes and filename from artifact storage.
    
    Retrieves a file from EliteA artifact storage using its filepath.
    This is a shared utility used across multiple toolkits (Jira, Confluence, SharePoint, etc.).
    
    Args:
        elitea: EliteA client instance (should have artifact() method)
    ...
    """
    if not elitea:
        raise ValueError("EliteA client is required for artifact operations")
    
    # Get artifact client - bucket name doesn't matter for download by filepath
    artifact_client = elitea.artifact('__temp__')
