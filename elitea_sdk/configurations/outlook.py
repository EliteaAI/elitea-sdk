...
class OutlookConfiguration(BaseModel):
    model_config = ConfigDict(
        json_schema_extra={
            "metadata": {
                ...
                "type": "outlook",
                "categories": ["office"],
                "extra_categories": ["outlook", "microsoft", "email", "mail"],
            }
        }
    )
    ...
    def check_connection(settings: dict) -> str | None:
        ...
        log.debug(f"Checking Outlook connection with settings: {safe_settings}")

        oauth_discovery_endpoint = settings.get("oauth_discovery_endpoint")
        if not oauth_discovery_endpoint:
            return "OAuth discovery endpoint is required for Outlook"

        return OutlookConfiguration._check_connection_delegated(settings, oauth_discovery_endpoint)
    ...
    def _build_mcp_authorization_required(
        ...
        resource_metadata: Dict = {
            "resource_name": "Outlook",
            "resource": "https://graph.microsoft.com/Mail",
