...
Sessions are persisted to $ELITEA_DIR/sessions/<session_id>/
...
def get_sessions_dir() -> Path:
    """Get the sessions directory path (relative to $ELITEA_DIR or .elitea)."""
    elitea_dir = os.environ.get('ELITEA_DIR', '.elitea')
    return Path(elitea_dir) / 'sessions'
...
def get_elitea_dir() -> Path:
    """Get the ELITEA_DIR path (relative to $ELITEA_DIR or .elitea)."""
    return Path(os.environ.get('ELITEA_DIR', '.elitea'))
...
def to_portable_path(path: str) -> str:
    ...
    If the path is under $ELITEA_DIR, store as relative path (e.g., 'agents/my-agent.yaml').
    ...
    Returns:
        Portable path string (relative to ELITEA_DIR if applicable, else absolute)
    """
    if not path:
        return path
    ...
    try:
        ...
        elitea_dir = get_elitea_dir().resolve()
        
        # Check if path is under ELITEA_DIR
        if str(path_obj).startswith(str(elitea_dir)):
            relative = path_obj.relative_to(elitea_dir)
            return str(relative)
...
def from_portable_path(portable_path: str) -> str:
    ...
    If the path is relative, resolve it against $ELITEA_DIR.
    ...
    # Resolve relative path against ELITEA_DIR
    elitea_dir = get_elitea_dir()
    return str(elitea_dir / portable_path)
...
from elitea_sdk.runtime.middleware.planning import (
    PlanStep,
    PlanState,
    StepStatus,
    PlanningMiddleware,
...
def load_plan_state(session_id: str) -> Optional[PlanState]:
    ...
    try:
        ...
        from elitea_sdk.runtime.middleware.planning import FilesystemStorage
