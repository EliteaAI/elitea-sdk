...
def _get_elitea_system_prompt(config) -> str:
    """
    Get the EliteA system prompt from user config or fallback to default.
    
    Checks for $ELITEA_DIR/agents/default.agent.md first, then falls back
    to the built-in DEFAULT_PROMPT.
    
    Returns:
        The system prompt string for EliteA
    """
    ...
    if custom_prompt_path.exists():
        try:
            ...
            logger.debug(f"Failed to load custom EliteA prompt from {custom_prompt_path}: {e}")
...
def _get_inventory_system_prompt(config) -> str:
    ...
    Checks for $ELITEA_DIR/agents/inventory.agent.md first, then falls back
...
def _resolve_inventory_path(path: str, work_dir: Optional[str] = None) -> Optional[str]:
    ...
    3. Relative to .elitea/inventory/ in current directory
    4. Relative to .elitea/inventory/ in work_dir (if different)
    ...
    # Try .elitea/inventory/ in current directory
    elitea_inventory_path = cwd / '.elitea' / 'inventory' / path
    if elitea_inventory_path.exists():
        return str(elitea_inventory_path.resolve())
    ...
    if work_dir:
        ...
        if work_path != cwd:
            ...
            # Try .elitea/inventory/ in work_dir
            work_elitea_path = work_path / '.elitea' / 'inventory' / path
            if work_elitea_path.exists():
                return str(work_elitea_path.resolve())
...
def _build_inventory_config(path: str, work_dir: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    # Extract toolkit name from filename (e.g., 'elitea' from 'elitea.json')
...
def _get_inventory_json_files(work_dir: Optional[str] = None) -> List[str]:
    ...
    2. .elitea/inventory/ directory (*.json files)
    3. work_dir and work_dir/.elitea/inventory/ if different from cwd
    ...
    # .elitea/inventory/ directory
    elitea_inv = cwd / '.elitea' / 'inventory'
    if elitea_inv.exists():
        for f in elitea_inv.glob('*.json'):
            display = f'.elitea/inventory/{f.name}'
    ...
    if work_dir:
        ...
        if work_path != cwd:
            ...
            work_elitea_inv = work_path / '.elitea' / 'inventory'
            if work_elitea_inv.exists():
                for f in work_elitea_inv.glob('*.json'):
                    display = f'.elitea/inventory/{f.name}'
                    if display not in seen:
                        suggestions.append(display)
                        seen.add(display)
...
def _select_toolkit_interactive(config) -> Optional[Dict[str, Any]]:
    """
    Show interactive menu to select a toolkit from $ELITEA_DIR/tools.
...
def _list_available_toolkits(config) -> List[str]:
    """
    List names of all available toolkits in $ELITEA_DIR/tools.
...
def _find_toolkit_by_name(config, toolkit_name: str) -> Optional[Dict[str, Any]]:
    """
    Find a toolkit by name in $ELITEA_DIR/tools.
...
def _select_agent_interactive(client, config) -> Optional[str]:
    ...
    # Built-in agents
    console.print(f"1. [[bold]💬 EliteA[/bold]] [cyan]Chat directly with LLM (no agent)[/cyan]")
    ...
    while True:
        try:
            ...
            if choice == '1':
                console.print(f"✓ [green]Selected:[/green] [bold]EliteA[/bold]")
                return '__direct__'
...
@click.option('--inventory', 'inventory_path', type=str,
              help='Load inventory/knowledge graph from JSON file (e.g., elitea.json or .elitea/inventory/elitea.json)')
...
def agent_chat(ctx, agent_source: Optional[str], version: Optional[str], 
    ...
    \b
    Examples:
      elitea chat                      # Interactive agent selection
      elitea chat my-agent             # Chat with platform agent
      elitea chat ./agent.md           # Chat with local agent file
      elitea chat --inventory elitea.json
      elitea chat my-agent --dir ./src
      elitea chat my-agent --thread-id abc123
      elitea chat my-agent -v quiet    # Hide tool calls
      elitea chat my-agent -v debug    # Show all LLM calls
      elitea chat __inventory__ --toolkit-config jira.json
    """
    formatter = ctx.obj['formatter']
    config = ctx.obj['config']
    client = get_client(ctx)
    ...
    try:
        ...
        if inventory_path:
            ...
            if inventory_config:
                ...
                console.print("[dim]Searched in current directory and .elitea/inventory/[/dim]")
        ...
        if is_direct:
            # Direct chat mode - no agent, just LLM with EliteA instructions
            agent_name = "EliteA"
            agent_type = "Direct LLM"
            elitea_prompt = _get_elitea_system_prompt(config)
            agent_def = {
                'model': model or default_model,
                'temperature': temperature if temperature is not None else default_temperature,
                'max_tokens': max_tokens or default_max_tokens,
                'system_prompt': elitea_prompt
            }
        elif is_inventory:
                ...
                if user_input == '/inventory' or user_input.startswith('/inventory '):
                    ...
                    if len(parts) == 1:
                        ...
                        if not inventory_config:
                            ...
                            console.print(f"[dim]  - {Path.cwd() / '.elitea' / 'inventory'}[/dim]")
                            if primary_dir:
                                console.print(f"[dim]  - {primary_dir}[/dim]")
                                console.print(f"[dim]  - {Path(primary_dir) / '.elitea' / 'inventory'}[/dim]")
                            continue
                ...
                if user_input == '/agent':
                    ...
                    if selected_agent and selected_agent != '__direct__' and selected_agent != '__inventory__':
                        ...
                        try:
                            ...
                            console.print(Panel(
                                f"[cyan]ℹ Switched to [bold]EliteA[/bold]. Agent state reset, chat history preserved.[/cyan]",
                                border_style="cyan",
                                box=box.ROUNDED
                            ))
                        except Exception as e:
...
def agent_run(ctx, agent_source: str, message: str, version: Optional[str],
    ...
    \b
    Examples:
      elitea run my-agent "What is the status of JIRA-123?"
      elitea run ./agent.md "Create a new toolkit for Stripe API"
      elitea -o json run my-agent "Search for bugs" --toolkit-config jira.json
      elitea run my-agent "Analyze code" --dir ./myproject
      elitea run my-agent "Start task" --save-thread thread.txt
      elitea run my-agent "Query" -v quiet
      elitea run my-agent "Query" -v debug
    """
    formatter = ctx.obj['formatter']
    client = get_client(ctx)
            ...
            if agent_executor:
                ...
                try:
                    ...
                    console.print(Panel(
                        ...
                        f"• Use [cyan]elitea agent chat[/cyan] for interactive continuation\n"
...
@click.option(
    ...
    'agent_source',
    required=False,
    default=str(Path('.elitea') / 'agents' / 'test-runner.agent.md'),
    show_default=True,
...
@click.option('--results-dir', required=False, default=str(Path('.elitea') / 'tests' / 'results'),
...
@click.option('--temperature', type=float, help='Override temperature')
@click.option('--max-tokens', type=int, help='Override max tokens')
@click.option('--dir', 'work_dir', required=False, default=str(Path('.elitea')),
              type=click.Path(exists=True, file_okay=False, dir_okay=True),
              help='Grant agent filesystem access to this directory')
@click.option('--data-generator', required=False, default=str(Path('.elitea') / 'agents' / 'test-data-generator.agent.md'),
              type=click.Path(exists=True),
              help='Path to test data generator agent definition file')
@click.option('--validator', type=click.Path(exists=True),
              help='Path to test validator agent definition file (default: .elitea/agents/test-validator.agent.md)')
...
def execute_test_cases(ctx, agent_source: str, test_cases_dir: str, results_dir: str,
    ...
    \b
    Examples:
      elitea agent execute-test-cases --test-cases-dir ./tests --results-dir ./results
      elitea agent execute-test-cases --agent_source ./agent.json --test-cases-dir ./tests --results-dir ./results \
