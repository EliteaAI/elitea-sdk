...
Usage:
    ...
    elitea inventory presets
    
    # Ingest using a preset (recommended!)
    elitea inventory ingest --dir ./my-project --graph ./graph.json --preset python
    
    # Ingest using a toolkit config file
    elitea inventory ingest --toolkit .elitea/tools/github.json --graph ./graph.json -w "*.md"
    
    # Ingest from a local git repository
    elitea inventory ingest --source localgit --path /path/to/repo --graph ./graph.json
    
    # Use a config file for LLM/embedding/guardrails settings
    elitea inventory ingest --toolkit ./github.json -g ./graph.json --config ingestion-config.yml
    
    # Check ingestion status (failed files, progress)
    elitea inventory status --graph ./graph.json --name my-source
    
    # Retry failed files from previous ingestion
    elitea inventory retry --dir ./my-project -g ./graph.json --name my-source
    elitea inventory retry --dir ./my-project -g ./graph.json --name my-source --force
    
    # Generate config template
    elitea inventory init-config
    
    # Show graph stats
    elitea inventory stats --graph ./graph.json
    
    # Search the graph
    elitea inventory search "PaymentService" --graph ./graph.json
"""
...
def presets():
    ...
    Example:
        elitea inventory presets
    """
    from elitea_sdk.community.inventory import list_presets, get_preset
    ...
    click.echo("Usage:")
    click.echo("  elitea inventory ingest --dir ./my-project -g ./graph.json --preset python")
    click.echo("  elitea inventory ingest --dir ./src -g ./graph.json -p typescript -w '*.json'")
    click.echo()
...
def init_config(output: str):
    ...
    Example:
        elitea inventory init-config -o ./my-config.yml
    """
    from elitea_sdk.community.inventory import generate_config_template
...
@click.option('--toolkit', '-t', type=click.Path(exists=True),
              help='Path to toolkit config JSON (e.g., .elitea/tools/github.json)')
...
def ingest(ctx, toolkit: Optional[str], directory: Optional[str], graph: str, 
    ...
    \b
    Examples:
      elitea inventory ingest --dir ./src -g graph.json --preset python
      elitea inventory ingest --dir ./src -g graph.json -w "*.py" -w "*.md"
      elitea inventory ingest --dir ./src -g graph.json -p typescript -w "*.json"
      elitea inventory ingest --dir ./docs -g graph.json --name my-docs
      elitea inventory ingest -t github.json -g graph.json -w "*.md"
      elitea inventory ingest --dir ./src -g graph.json -c config.yml
    """
    ...
    if preset:
        from elitea_sdk.community.inventory import get_preset, list_presets
    ...
    try:
        from elitea_sdk.community.inventory import IngestionPipeline, IngestionConfig
        ...
        if result.success:
            ...
            if result.failed_documents:
                ...
                click.echo(f"   Run 'elitea inventory status -g {graph} -n {source_name}' to see details")
                click.echo(f"   Run 'elitea inventory retry ...' to retry failed files")
        else:
            click.echo(f"\n❌ Ingestion failed!")
...
def retry(ctx, toolkit: Optional[str], directory: Optional[str], graph: str,
    ...
    \b
    Examples:
      elitea inventory retry --dir ./src -g graph.json -n my-source
      elitea inventory retry --dir ./src -g graph.json -n my-source --force
      elitea inventory retry -t github.json -g graph.json -n github-repo
    """
    # Validate: must have either --toolkit or --dir
    if not toolkit and not directory:
        raise click.ClickException("Must specify either --toolkit or --dir")
    ...
    try:
        ...
        if result.success:
            ...
            if result.failed_documents:
                ...
                click.echo(f"   Run 'elitea inventory status -g {graph} -n {name}' to see details")
            else:
...
def status(graph: str, name: str):
    ...
    Example:
        elitea inventory status -g ./graph.json -n my-source
    """
    checkpoint_path = _get_checkpoint_path(graph, name)
    ...
    if failed_files:
        ...
        click.echo(f"      elitea inventory retry --dir <path> -g {graph} -n {name}")
        click.echo(f"      elitea inventory retry --dir <path> -g {graph} -n {name} --force")
...
def stats(graph: str):
    ...
    Example:
        elitea inventory stats -g ./graph.json
    """
    try:
        from elitea_sdk.community.inventory import KnowledgeGraph
...
def search(query: str, graph: str, entity_type: Optional[str], limit: int):
    ...
    Example:
        elitea inventory search "Payment" -g ./graph.json
        elitea inventory search "User" -g ./graph.json --type class
    """
    try:
        from elitea_sdk.community.inventory import KnowledgeGraph
...
def entity(name: str, graph: str, relations: bool):
    ...
    Example:
        elitea inventory entity "PaymentProcessor" -g ./graph.json
    """
    try:
        from elitea_sdk.community.inventory import InventoryRetrievalApiWrapper
...
def impact(name: str, graph: str, direction: str, depth: int):
    ...
    Example:
        elitea inventory impact "UserService" -g ./graph.json
        elitea inventory impact "Database" -g ./graph.json --direction upstream
    """
    try:
        from elitea_sdk.community.inventory import InventoryRetrievalApiWrapper
...
def visualize(graph: str, output: Optional[str], open_browser: bool, title: Optional[str]):
    ...
    Example:
        elitea inventory visualize -g ./graph.json
        elitea inventory visualize -g ./graph.json -o my_graph.html
        elitea inventory visualize -g ./graph.json --no-open
    """
    try:
        from elitea_sdk.community.inventory.visualize import generate_visualization
        from elitea_sdk.community.inventory import KnowledgeGraph
        import webbrowser
        import os
...
def enrich(graph: str, output: Optional[str], deduplicate: bool, cross_source: bool,
    ...
    Example:
        elitea inventory enrich -g ./graph.json
        elitea inventory enrich -g ./graph.json -o enriched.json
        elitea inventory enrich -g ./graph.json --deduplicate
        elitea inventory enrich -g ./graph.json --dry-run
    """
    try:
        from elitea_sdk.community.inventory.enrichment import GraphEnricher
...
def _load_toolkit_config(toolkit_path: str) -> Dict[str, Any]:
    """Deprecated: Use elitea_sdk.community.inventory.toolkit_utils.load_toolkit_config instead."""
    from elitea_sdk.community.inventory.toolkit_utils import load_toolkit_config
    return load_toolkit_config(toolkit_path)


def _get_llm(ctx, model: Optional[str] = None, temperature: float = 0.0):
    """Deprecated: Use elitea_sdk.community.inventory.toolkit_utils.get_llm_for_config instead."""
    from .cli import get_client
    from elitea_sdk.community.inventory.toolkit_utils import get_llm_for_config
...
def _get_source_toolkit(toolkit_config: Dict[str, Any]):
    """Deprecated: Use elitea_sdk.community.inventory.toolkit_utils.get_source_toolkit instead."""
    from elitea_sdk.community.inventory.toolkit_utils import get_source_toolkit
