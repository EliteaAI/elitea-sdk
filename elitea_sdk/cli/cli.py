"""
Main CLI application for EliteA SDK.
...
def cli(ctx, env_file: str, debug: bool, verbose: bool, output: str):
    """
    EliteA SDK CLI - Test agents and toolkits from the command line.
    ...
    if debug:
        logging.getLogger('elitea_sdk').setLevel(logging.DEBUG)
        logger.setLevel(logging.DEBUG)
        logger.debug("Debug logging enabled")
    elif verbose:
        # Verbose mode shows INFO level (timing info)
        logging.getLogger('elitea_sdk').setLevel(logging.INFO)
...
def get_client(ctx):
    """
    Get configured EliteAClient from context.
    ...
    try:
        ...
        logger.debug(f"EliteAClient initialized for project {config.project_id}")
        return client
    except Exception as e:
        raise click.ClickException(f"Failed to initialize EliteAClient: {str(e)}")
