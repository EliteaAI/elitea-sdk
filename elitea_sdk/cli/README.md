# EliteA CLI User Guide

The EliteA CLI is your command-line interface for building and running AI agents with powerful integrations.
...
```bash
ELITEA_PROJECT_NAME=my-project
ELITEA_DEPLOYMENT=https://domain.elitea.ai
ELITEA_AUTH_TOKEN=your-token-here
...
Create `.elitea/agents/hello.agent.md`:
...
```bash
elitea agent chat .elitea/agents/hello.agent.md
```
...
```bash
# Show platform agent
elitea agent show my-agent

# Show local agent
elitea agent show .elitea/agents/my-agent.agent.md
```
...
elitea agent chat my-agent

# Chat with local agent
elitea agent chat .elitea/agents/my-agent.agent.md

# Override settings
elitea agent chat my-agent --model gpt-4-turbo --temperature 0.5

# Mount a directory for filesystem/terminal access
elitea agent chat my-agent --dir ./my-project
```
...
elitea agent run my-agent "What is the weather today?"

# Get JSON output (for scripting)
elitea --output json agent run my-agent "Get status"
```
...
**File**: `.elitea/agents/my-agent.agent.md`
...
elitea-cli agent chat code-agent --dir ./my-project
...
📋 Saved Sessions:
  ...
  ● 20251128-120000-c3d4
      EliteA (gpt-4o) • 2025-11-28 12:00
...
**At startup:**
```bash
elitea agent chat my-agent --mode auto
```
...
Create `~/.elitea/blocked_patterns.txt` to add your own:
...
elitea toolkit show jira

# Generate configuration template
elitea toolkit config jira > .elitea/tools/jira.json
```
...
**JSON Format** (`.elitea/tools/jira.json`):
```json
{
  "toolkit_type": "jira",
  "toolkit_name": "jira",
...
**YAML Format** (`.elitea/tools/github.yaml`):
```yaml
toolkit_type: github
toolkit_name: github
credentials:
...
```bash
elitea agent chat jira-agent \
  --toolkit-config .elitea/tools/jira.json \
  --toolkit-config .elitea/tools/github.json
```
...
**1. Create MCP configuration** (`.elitea/mcp.json`):
...
**2. Create agent with MCP tools** (`.elitea/agents/browser-agent.agent.md`):
...
```bash
elitea agent chat browser-agent
...
**Agent**: `.elitea/agents/code-reviewer.agent.md`
...
**Usage**:
```bash
elitea agent chat code-reviewer --dir ./my-project
...
**Agent**: `.elitea/agents/jira-assistant.agent.md`
...
```bash
# Interactive
elitea agent chat jira-assistant --toolkit-config .elitea/tools/jira.json
> What are the open bugs in PROJ?
> Create a new task for fixing the login issue

# Single command
elitea agent run jira-assistant "List all high-priority tickets" \
  --toolkit-config .elitea/tools/jira.json
```

### Example 3: Web Research Agent

**MCP Config**: `.elitea/mcp.json`
...
**Agent**: `.elitea/agents/researcher.agent.md`
...
**Agent**: `.elitea/agents/doc-generator.agent.md`
...
**Usage**:
```bash
elitea agent run doc-generator \
...
**Agent**: `.elitea/agents/testing-agent.agent.md`
...
**Usage**:
```bash
elitea agent chat testing-agent --dir ./my-project --mode auto
...
2. Check you're using the latest version: `pip install --upgrade elitea-sdk`
...
```bash
elitea agent chat my-agent --max-tokens 4000
```

### "Command blocked by security policy"

**Solution:**
The command matches a blocked pattern. Check `~/.elitea/blocked_patterns.txt`:
...
**In single-run mode (`elitea agent run`):**
The command will stop with suggestions:
- Use `elitea agent chat` for interactive continuation
...
3. Session files are in `~/.elitea/sessions/`
...
Sessions are stored at `~/.elitea/sessions/<session_id>/`:
...
1. **Implement toolkit** in `elitea_sdk/toolkits/`:
```python
class MyToolkit:
...
1. Add server to `.elitea/mcp.json`
...
# Test specific agent
elitea agent run test-agent "Hello" --verbose debug
```
...
- **Issues**: [GitHub Issues](https://github.com/EliteaAI/elitea.github.io/issues)
