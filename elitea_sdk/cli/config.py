"""
Configuration management for EliteA CLI.
...
class CLIConfig:
    """Configuration manager for EliteA CLI."""
    ...
    def __init__(self, env_file: Optional[str] = None):
        ...
        Args:
            env_file: Path to .env file. If None, checks ELITEA_ENV_FILE env var,
                      then falls back to .elitea/.env or .env in current directory
        """
        ...
        if env_file:
            ...
            # Check ELITEA_ENV_FILE environment variable first
            elitea_env_file = os.getenv('ELITEA_ENV_FILE')
            if elitea_env_file:
                # Expand ~ and resolve path
                expanded_path = os.path.expanduser(elitea_env_file)
                if os.path.exists(expanded_path):
                    self.env_file = expanded_path
                else:
                    logger.warning(f"ELITEA_ENV_FILE set to {elitea_env_file} but file not found")
                    self.env_file = expanded_path  # Still use it, will warn later
            elif os.path.exists(os.path.expanduser('~/.elitea/.env')):
                self.env_file = os.path.expanduser('~/.elitea/.env')
            elif os.path.exists('.elitea/.env'):
                self.env_file = '.elitea/.env'
            else:
                self.env_file = '.env'
        self._load_env()
    ...
    def _load_config_json(self):
        """Load configuration from $ELITEA_DIR/config.json as fallback."""
        # Try ELITEA_DIR from env, then ~/.elitea, then .elitea
        elitea_dir = os.getenv('ELITEA_DIR')
        if elitea_dir:
            config_path = os.path.join(os.path.expanduser(elitea_dir), 'config.json')
        else:
            # Try ~/.elitea/config.json first, then .elitea/config.json
            home_config = os.path.expanduser('~/.elitea/config.json')
            local_config = '.elitea/config.json'
    ...
    def default_model(self) -> Optional[str]:
        ...
        return self._get_config_value('ELITEA_DEFAULT_MODEL', 'default_model')
    
    @property
    def default_temperature(self) -> Optional[float]:
        """Get default temperature from environment or config.json."""
        try:
            value = self._get_config_value('ELITEA_DEFAULT_TEMPERATURE', 'default_temperature')
    ...
    def default_max_tokens(self) -> Optional[int]:
        ...
        try:
            value = self._get_config_value('ELITEA_DEFAULT_MAX_TOKENS', 'default_max_tokens')
            return int(value) if value is not None else None
        except (TypeError, ValueError):
            return None
    
    @property
    def elitea_dir(self) -> str:
        """Get EliteA directory from environment (defaults to .elitea)."""
        return os.getenv('ELITEA_DIR', '.elitea')
    
    @property
    def agents_dir(self) -> str:
        """Get agents directory (derived from ELITEA_DIR)."""
        elitea_agents = os.path.join(self.elitea_dir, 'agents')
        # Fallback to .github/agents if .elitea/agents doesn't exist
        if self.elitea_dir == '.elitea' and not os.path.exists(elitea_agents):
            if os.path.exists('.github/agents'):
                return '.github/agents'
        return elitea_agents
    
    @property
    def tools_dir(self) -> str:
        """Get tools directory (derived from ELITEA_DIR)."""
        return os.path.join(self.elitea_dir, 'tools')
    
    @property
    def mcp_config_path(self) -> str:
        """Get MCP configuration path (derived from ELITEA_DIR)."""
        elitea_mcp = os.path.join(self.elitea_dir, 'mcp.json')
        # Fallback to mcp.json in current directory
        if not os.path.exists(elitea_mcp) and os.path.exists('mcp.json'):
            return 'mcp.json'
        return elitea_mcp
    ...
    def sessions_dir(self) -> str:
        """Get sessions directory (derived from ELITEA_DIR)."""
        return os.path.join(self.elitea_dir, 'sessions')
