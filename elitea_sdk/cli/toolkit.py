"""
Toolkit testing commands for EliteA CLI.
...
def toolkit_list(ctx, failed: bool):
    ...
    try:
        ...
        from elitea_sdk.tools import AVAILABLE_TOOLS, AVAILABLE_TOOLKITS, FAILED_IMPORTS
...
def toolkit_schema(ctx, toolkit_name: str):
    ...
    try:
        ...
        for name, cls in AVAILABLE_TOOLKITS.items():
            if name.lower().replace('toolkit', '').replace('elitea', '').strip() == toolkit_name.lower():
                toolkit_class = cls
                break
        ...
        if not toolkit_class:
            available = [name.lower().replace('toolkit', '').replace('elitea', '').strip() 
...
def toolkit_test(ctx, toolkit_type: str, tool: str, config_file, params, 
    ...
    \b
    Examples:
      elitea toolkit test jira --tool get_issue --config jira.json --params params.json
      elitea toolkit test jira --tool get_issue --config jira.json --param issue_key=PROJ-123
      elitea -o json toolkit test github --tool get_issue --config github.json
    """
    formatter = ctx.obj['formatter']
    ...
    try:
        ...
        if toolkit_type not in AVAILABLE_TOOLS:
            raise click.ClickException(
                ...
                f"Use 'elitea-cli toolkit list' to see available toolkits."
            )
        ...
        try:
            ...
            click.echo("Use 'elitea-cli toolkit schema {toolkit_type}' to see configuration options")
