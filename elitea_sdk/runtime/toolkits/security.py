...
def _load_blocklist_from_env() -> None:
    ...
    env_toolkits = os.environ.get('ELITEA_BLOCKED_TOOLKITS', '')
    env_tools = os.environ.get('ELITEA_BLOCKED_TOOLS', '')
    ...
    if env_tools:
        try:
            ...
            logger.warning(f"[SECURITY] Failed to parse ELITEA_BLOCKED_TOOLS: {e}")
...
def _load_sensitive_tools_from_env() -> None:
    ...
    env_sensitive_tools = os.environ.get('ELITEA_SENSITIVE_TOOLS', '')
    env_company_name = os.environ.get('ELITEA_SENSITIVE_ACTION_COMPANY_NAME', '')
    env_message_template = os.environ.get('ELITEA_SENSITIVE_ACTION_MESSAGE_TEMPLATE', '')
