...
def get_tools(tools_list: list, elitea_client=None, llm=None, memory_store=None):
    """
    Get sandbox tools for the provided tool configurations.

    Args:
        tools_list: List of tool configurations
        elitea_client: EliteA client instance for sandbox tools
...
class PyodideSandboxTool(BaseTool):
    ...
    def __init__(self, **kwargs: Any) -> None:
        # Ensure display metadata is set so EliteACallback can render the FE chip correctly.
    ...
    def invoke(self, input, config=None, **kwargs):
        ...
        toolkit_name, display_name) reach EliteACallback.on_tool_start via
    ...
    def _run(self, code: str) -> str:
        ...
        try:
            ...
            # Note: _arun() handles code preparation (elitea_client injection)
...
def create_sandbox_tool(stateful: bool = False, allow_net: bool = True, elitea_client: Optional[Any] = None) -> BaseTool:
    """
    ...
    """
    if stateful:
        return StatefulPyodideSandboxTool(allow_net=allow_net, elitea_client=elitea_client)
    else:
        return PyodideSandboxTool(stateful=False, allow_net=allow_net, elitea_client=elitea_client)
...
class SandboxToolkit(BaseToolkit):
    ...
    @classmethod
    def get_toolkit(cls, stateful: bool = False, allow_net: bool = True, elitea_client=None, **kwargs):
        """
        ...
        if stateful:
            tools.append(StatefulPyodideSandboxTool(allow_net=allow_net, elitea_client=elitea_client))
        else:
            tools.append(PyodideSandboxTool(stateful=False, allow_net=allow_net, elitea_client=elitea_client))
