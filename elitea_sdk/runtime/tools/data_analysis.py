"""
Data Analysis internal tool for EliteA SDK.
...
def get_tools(tools_list: list, elitea_client=None, llm=None, memory_store=None):
    ...
    Args:
        ...
        elitea_client: EliteA client instance (required for data analysis)
        ...
        if (tool.get('type') == 'data_analysis' or
            ...
            try:
                if not elitea_client:
                    logger.error("EliteA client is required for data analysis tools")
                    continue
...
class DataAnalysisToolkit(BaseToolkit):
    ...
    def toolkit_config_schema() -> Type[BaseModel]:
        ...
        from elitea_sdk.tools.pandas.api_wrapper import PandasWrapper
    ...
    def get_toolkit(
        ...
        **kwargs
        ...
        Args:
            elitea_client: EliteA client instance (required)
            llm: LLM for code generation (optional, uses elitea_client.llm if not provided)
        ...
        Raises:
            ValueError: If elitea_client or bucket_name is not provided
        """
        if not elitea_client:
            raise ValueError("EliteA client is required for data analysis")
