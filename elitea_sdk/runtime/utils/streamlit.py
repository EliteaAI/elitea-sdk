...
# from elitea_sdk.runtime.llms.elitea import EliteAChatModel
        ...
        try:
            ...
            try:
                if not st.session_state.client:
                    raise ValueError("EliteA client not available")
    ...
    with st.sidebar:
        ...
        with st.expander(login_title, expanded=st.session_state.login_form_expanded):
            ...
            with st.form("settings_form", clear_on_submit=False):
                ...
                if submitted:
                    with st.spinner("Logging to EliteA..."):
                        try:

                            st.session_state.client = EliteAClient(
                                base_url=deployment,
                            ...
                            st.success("✅ Successfully logged in to EliteA!")
        ...
        # Main tabs
        llmconfig, toolkit_config = st.tabs(["EliteA Agents", "Toolkit Testing"])
        ...
        with toolkit_config:
            ...
            **Welcome to Toolkit Testing!** This interface allows you to:
            ...
            if not st.session_state.client:
                ...
                💡 **Tip:** You can find your API key and deployment URL in your EliteA dashboard.
                """)
                st.stop()
            ...
            if st.session_state.client:
                ...
                st.info("You need to login to your EliteA deployment before you can test toolkits.")
                st.markdown("👈 Please use the **EliteA Login Form** in the sidebar to authenticate.")
    ...
    if st.session_state.client and st.session_state.agent_executor and st.session_state.agent_chat:
        ...
        if prompt := st.chat_input():
            ...
            with st.chat_message("assistant", avatar=ai_icon):
                st_cb = EliteAStreamlitCallback(st)
        ...
        col1, col2 = st.columns([3, 1])
        with col1:
            st.info("💡 **Enhanced Testing:** Using `EliteAClient.test_toolkit_tool()` method with event capture and runtime callbacks.")
        with col2:
        ...
        if st.session_state.client:
            st.title("🎯 EliteA SDK Toolkit Interface")
            st.markdown("""
            ### Welcome to the EliteA SDK!
            
            **Choose your path:**
            
            🤖 **Agent Chat**: Load an EliteA agent for AI-powered conversations
            - Go to **EliteA Agents** tab in the sidebar
            ...
            st.title("🔐 Please Login First")
            st.info("You need to login to your EliteA deployment before you can use the interface.")
            st.markdown("👈 Please use the **EliteA Login Form** in the sidebar to authenticate.")
