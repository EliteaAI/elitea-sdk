...
class DiscoveryMode(Enum):
    """MCP discovery modes."""
    STATIC = "static"      # Use elitea.get_mcp_toolkits() registry
...
class McpManager:
    ...
    async def discover_server_tools(
        ...
        **kwargs
        ...
        Args:
            ...
            elitea_client: EliteA client (required for static mode)
        ...
        if discovery_mode == DiscoveryMode.DYNAMIC:
            ...
            return await self._discover_static(server_name, elitea_client)

        elif discovery_mode == DiscoveryMode.HYBRID:
            return await self._discover_hybrid(server_name, connection_config, elitea_client)
    ...
    async def _discover_static(
        ...
        if not elitea_client or not hasattr(elitea_client, 'get_mcp_toolkits'):
            raise ValueError("EliteA client with get_mcp_toolkits() required for static discovery")

        try:
            # Use existing registry approach
            all_toolkits = elitea_client.get_mcp_toolkits()
    ...
    async def _discover_hybrid(
        ...
        if self._static_fallback_enabled and elitea_client:
            try:
                return await self._discover_static(server_name, elitea_client)
            except Exception as e:
...
async def discover_mcp_tools(
    ...
    Args:
        ...
        elitea_client: EliteA client (for static discovery)
