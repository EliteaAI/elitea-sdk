"""Integration test for the image-only user_prompt extra_params key (#6530).

Verifies that read_file's extra_params={"user_prompt": "..."} reaches
...
def test_user_prompt_reaches_image_loader_prompt():
    captured = {}
    ...
    with patch.object(EliteAImageLoader, "get_content", fake_get_content):
        result = parse_file_content(
            ...
            extra_params={"user_prompt": "Describe only the red circle"},
        )
...
def test_default_prompt_used_when_user_prompt_absent():
    captured = {}
...
def test_user_prompt_is_noop_for_non_image_file():
    # A .txt file goes through EliteATextLoader, which has no `prompt` concept
    # at all — user_prompt must be silently ignored, not raise or leak.
    result = parse_file_content(
        file_name="notes.txt",
