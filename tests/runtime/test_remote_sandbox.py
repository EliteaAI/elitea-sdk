...
from elitea_sdk.runtime.langchain.remote_sandbox import RemoteSandbox
...
@pytest.mark.asyncio
async def test_service_unavailable_503(sandbox):
...
@pytest.mark.asyncio
async def test_timeout_passed_in_body(sandbox):
    response = MockResponse(200, {"success": True})
    session = MockSession(response)
    ...
    assert session._last_json["timeout_seconds"] == 30
