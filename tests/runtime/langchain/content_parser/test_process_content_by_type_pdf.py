"""
Integration tests for EliteAPDFLoader as invoked through the production path
    ...
    base_indexer_toolkit._apply_loaders_chunkers(document)
        → process_document_by_type(content=pdf_bytes, extension_source="file.pdf",
            ...
            → process_content_by_type(content, filename, llm, chunking_config)
                → EliteAPDFLoader(file_path=tmpfile, **loader_kwargs).load()
...
KNOWN BUG
---------
EliteAPDFLoader.__init__ accepts **kwargs but never reads 'max_tokens'.
...
from elitea_sdk.tools.utils.content_parser import (
...
# ---------------------------------------------------------------------------
_PDF_FILES = _TEST_DATA / "EliteAPDFLoader" / "files"
...
class TestChunkingConfigPdf:
    ...
    that EliteATextLoader, EliteAJSONLoader, etc. already fulfill.

    KNOWN BUG: EliteAPDFLoader ignores max_tokens entirely. load() delegates
...
class TestLlmPdf:
    ...
    EliteAPDFLoader is instantiated.  load() uses PyPDFium2Loader — it never
    calls the LLM.  prompt and use_default_prompt have no effect in load().
    """

    def test_use_llm_false_llm_not_injected(self):
        """use_llm=False → llm kwarg must NOT reach EliteAPDFLoader."""
        content = _read_bytes(_PDF_TEXT_ONLY)
...
class TestMetadataPdf:
    """
    EliteAPDFLoader.load() sets chunk_id = doc.metadata['page'] (0-indexed).
    ...
    def test_load_sets_chunk_id_to_page_index(self):
        ...
        EliteAPDFLoader.load() copies it to metadata['chunk_id'].
        """
        docs = list(process_content_by_type(_read_bytes(_PDF_TEXT_ONLY), "doc.pdf"))
        assert len(docs) == 3
        # chunk_id should equal the 0-indexed page number
        for expected_page, doc in enumerate(docs):
            assert "chunk_id" in doc.metadata, "chunk_id must be set by EliteAPDFLoader.load()"
    ...
    def test_process_document_by_type_overrides_chunk_id_sequentially(self):
        ...
        value set by EliteAPDFLoader.load().
        """
        base_doc = Document(
            page_content="",
            metadata={
    ...
    def test_process_document_by_type_merges_base_metadata(self):
        ...
        for doc in result_docs:
            ...
            assert doc.metadata.get("project") == "elitea-sdk"
...
class TestHyperlinkPdf:
    """
    EliteAPDFLoader has two code paths:
