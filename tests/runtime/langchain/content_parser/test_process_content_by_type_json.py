...
chunking_config
  ...
  Unlike EliteAMarkdownLoader, EliteAJSONLoader reads max_tokens directly from
  **kwargs, so process_content_by_type wires it correctly.  These tests confirm
  ...
  Key implementation detail: EliteAJSONLoader passes max_tokens as max_chunk_size
...
_JSON_SIMPLE = _TEST_DATA / "EliteAJSONLoader" / "files" / "json_simple.json"
_JSON_LARGE  = _TEST_DATA / "EliteAJSONLoader" / "files" / "json_large.json"
_JSON_EMPTY  = _TEST_DATA / "EliteAJSONLoader" / "files" / "json_empty.json"
_JSON_ARRAY  = _TEST_DATA / "EliteAJSONLoader" / "files" / "json_array.json"
...
class TestChunkingConfigJson:
    ...
    EliteAJSONLoader reads max_tokens directly from **kwargs, so
