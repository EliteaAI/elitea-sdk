...
def pytest_sessionstart(session):
    ...
    indirectly traverse ``elitea_sdk.tools.chunkers`` and trigger a partial-init
    circular import window around ``loaders_map``. Preloading ``elitea_sdk.tools``
    once at session start avoids that timing issue in tests without changing
    production code paths.
    """
    import elitea_sdk.tools  # noqa: F401


class _SuppressOptionalToolkitImportWarnings(logging.Filter):
    """Hide optional toolkit import warnings during tests.

    Loader tests import chunkers through ``elitea_sdk.tools`` which may attempt
...
def pytest_configure(config):
    ...
    logging.getLogger("elitea_sdk.tools").addFilter(filter_instance)
