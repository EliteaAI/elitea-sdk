# EliteAMarkdownLoader Test Suite
...
This test suite validates the `EliteAMarkdownLoader` document loader implementation. The loader uses `markdown_chunker` to chunk Markdown text with header-aware splitting while respecting the `max_tokens` limit.
...
pytest tests/runtime/langchain/document_loaders/test_elitea_markdown_loader.py -v

# Run a specific input file
pytest tests/runtime/langchain/document_loaders/test_elitea_markdown_loader.py -v -k "markdown_large"
