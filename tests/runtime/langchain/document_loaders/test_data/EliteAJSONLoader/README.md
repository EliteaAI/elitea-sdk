...
This test suite validates the `EliteAJSONLoader` document loader implementation. The loader uses `RecursiveJsonSplitter` to chunk JSON data based on structural boundaries while respecting the `max_tokens` limit.
...
pytest tests/runtime/langchain/document_loaders/test_elitea_json_loader.py -v

# Run a specific input file
pytest tests/runtime/langchain/document_loaders/test_elitea_json_loader.py -v -k "json_large"
