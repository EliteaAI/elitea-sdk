...
## EliteAJSONLoader / EliteAJSONLinesLoader
...
**Last Updated:** March 19, 2026 (EliteAImageLoader: 60% → 80%)  
**Test Location:** `.elitea/tests/test_loaders/`
...
| **Loaders Complete** | 3 / 11 (EliteATextLoader, EliteAMarkdownLoader, EliteAJSONLinesLoader) |
| **Loaders Partial** | 3 / 11 (EliteACSVLoader 80%, EliteAJSONLoader 75%, EliteAImageLoader 80%) |
...
### ✅ EliteATextLoader — 100% Complete (5/5)
...
### ✅ EliteAMarkdownLoader — 100% Complete (4/4)
...
### ⚠️ EliteACSVLoader — 80% Complete (4/5)
...
### ✅ EliteAJSONLoader — 75% Complete (3/4)
...
### ✅ EliteAJSONLinesLoader — 100% Complete (JL04 + Extended Coverage)
...
**Pytest Implementation:** `test_elitea_jsonlines_loader.py` — **15/15 passed**
...
### ❌ EliteAExcelLoader — 0% Complete (0/6)
...
### ❌ EliteAPDFLoader — 0% Complete (0/5)
...
### ❌ EliteADocxMammothLoader — 0% Complete (0/4)
...
### ⚠️ EliteAImageLoader — 80% Complete (4/5 core + Extended Coverage)
...
| IL03 | JPEG | ✅ | elitea_screenshot_jpeg.json (3 configs), image_regular.json |
...
2. **elitea_screenshot_jpeg.json** - 3 configs covering:
   - OCR only (use_llm: false) ✅
   - LLM with default prompt
...
- ✅ elitea_screenshot.jpeg (JPEG - application screenshot)
...
**Pytest Implementation:** `test_elitea_image_loader.py` — **22/22 configs passing**
...
**Coverage Notes:**
- Uses real LLM (ChatAnthropic via DEPLOYMENT_URL/ELITEA_API_KEY)
...
### ❌ EliteAPowerPointLoader — 0% Complete (0/4)
...
### ❌ EliteADirectoryLoader — 0% Complete (0/9)
...
1. **EliteAExcelLoader** (6 tests) - High business value, commonly used format
2. **EliteAPDFLoader** (5 tests) - Essential document format
3. **EliteAJSONLoader** - Add malformed JSON test (JL05)
4. **EliteACSVLoader** - Verify CSV04 headers-only scenario
5. **EliteAImageLoader** - Complete remaining 1 core test (IL05 corrupted image) to reach 100%

**Note:** EliteAJSONLinesLoader 100% complete (15 tests passing), EliteAImageLoader 80% complete (22 tests passing)

### 🟡 Priority 1 - High (Core Functionality)

6. **EliteADocxMammothLoader** (4 tests) - Common document format
7. **EliteADirectoryLoader** (9 tests) - Complex but essential for bulk operations
   - Start with DIR01 (mixed-type), DIR04 (recursive), DIR07 (empty dir)

### 🟢 Priority 2 - Medium (Nice to Have)

8. **EliteAPowerPointLoader** (4 tests) - Less critical format

**Notes:**
- EliteAImageLoader at 80% complete → only IL05 (corrupted image) remaining for 100%
...
- [ ] Create EliteADocxMammothLoader directory
- [ ] Generate 3 DOCX files with various features
- [ ] Write 4 configurations for DL01-DL04
- [ ] Create EliteADirectoryLoader test structure
...
- [ ] Create EliteAImageLoader with LLM mocking
- [ ] Generate/collect 4 image files + 1 corrupted
- [ ] Write 5 configurations for IL01-IL05
- [ ] Create EliteAPowerPointLoader directory
...
```
.elitea/tests/test_loaders/
├── EliteATextLoader/          ✅ 8 tests (5 required + 3 extra)
│   ├── input/
│   │   ├── text_simple.json
│   │   ├── text_yaml.json
│   │   ├── text_latin1.json
...
│
├── EliteAMarkdownLoader/      ✅ 6 tests (4 required + 2 extra)
│   ├── input/
...
│
├── EliteACSVLoader/           ⚠️  7 tests (4-5 required + 2 extra)
│   ├── input/
│   │   ├── csv_simple.json
...
│       └── [6 corresponding files]
│
├── EliteAJSONLoader/          ⚠️  5 tests (4 required + 1 extra)
│   ├── input/
│   │   ├── json_simple.json
│   │   ├── json_nested.json
│   │   ├── json_array.json
...
│
└── EliteAJSONLinesLoader/     ✅ 15 tests (JL04 + extended)
    ├── input/
    ...
    ├── files/
    │   └── [5 corresponding files]
    └── pytest: test_elitea_jsonlines_loader.py ✅ 15/15 passed
```
...
- EliteATextLoader: 5/5 requirements (100%) ✅
- EliteAMarkdownLoader: 4/4 requirements (100%) ✅
- EliteAJSONLinesLoader: 1/1 requirement (100%) ✅
- EliteACSVLoader: 4/5 requirements (80%) ⚠️
- EliteAJSONLoader: 3/4 requirements (75%) ⚠️
- EliteAImageLoader: 3/5 requirements (60%) ⚠️ — **New: 14 test configs, pytest passing**
...
2. **Week 1:** Implement EliteAExcelLoader and EliteAPDFLoader (P0)
3. **Week 2:** Implement EliteADocxMammothLoader and EliteADirectoryLoader (P1)
4. **Week 3:** Implement EliteAImageLoader and EliteAPowerPointLoader (P2)
...
**March 19, 2026:**
- ✅ **EliteAImageLoader** implemented with 60% coverage (3/5 core requirements)
- ✅ 14 test configurations across 3 input files (image_regular, elitea_screenshot_jpeg, snail_bmp)
- ✅ test_elitea_image_loader.py created — all 14 configs passing
...
**March 13, 2026:**
- ✅ EliteAJSONLinesLoader fully implemented with 15 test cases
- ✅ test_elitea_jsonlines_loader.py created — all tests passing
