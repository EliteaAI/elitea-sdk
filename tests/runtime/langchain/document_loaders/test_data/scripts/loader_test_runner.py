...
def _get_llm_for_tests():
    ...
    EliteAClient credentials are not configured.
    
    Requires environment variables:
    - DEFAULT_LLM_MODEL_FOR_CODE_ANALYSIS: Model name
    - ELITEA_DEPLOYMENT_URL: EliteA deployment URL
...
def _load_documents_directly(loader_name: str, file_path: Path, config: Dict[str, Any], llm=None) -> List:
    ...
    """
    import importlib
    module = importlib.import_module(f'elitea_sdk.runtime.langchain.document_loaders.{loader_name}')
