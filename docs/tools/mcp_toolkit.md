...
The MCP Toolkit enables EliteA SDK to connect to a single remote MCP server and dynamically expose its tools within LangGraph agents. This toolkit provides a seamless way to integrate external tools and services following the [MCP specification](https://modelcontextprotocol.io/specification/2025-06-18).
...
- The `___` separator (TOOLKIT_SPLITTER) is used consistently across EliteA SDK
...
```python
from elitea_sdk.runtime.toolkits import McpToolkit
...
from elitea_sdk.runtime.toolkits import get_tools
...
# Get tools
tools = get_tools([mcp_config], elitea_client, llm)
...
1. **Server Not Found**
   ...
   ```
   - Verify the `server_name` matches the MCP server identifier in `elitea.get_mcp_toolkits()`
   - Check if the MCP server is properly registered with the EliteA client
...
4. **Tool Discovery Issues**
   ```
   WARNING: EliteA client does not support MCP toolkit discovery
   ```
   - Ensure your EliteA client has the `get_mcp_toolkits()` method implemented
...
```python
import logging
logging.getLogger('elitea_sdk.runtime.toolkits.mcp').setLevel(logging.DEBUG)
```
...
The MCP Toolkit is part of the EliteA SDK and follows the same licensing terms.

## References

- [Model Context Protocol Specification](https://modelcontextprotocol.io/specification/2025-06-18)
- [EliteA SDK Documentation](https://github.com/EliteaAI/elitea-sdk)
