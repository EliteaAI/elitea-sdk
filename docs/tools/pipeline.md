...
| `input` | list[string] | No | ['messages'] | Controls which state variables are injected into `elitea_state`. If empty or only `['messages']`, ALL state variables are injected. If specific variables are listed, ONLY those are injected (excluding 'messages') |
...
The Code Node automatically injects the current pipeline state into the execution environment as `elitea_state`. This allows your code to access and manipulate state variables.

**Selective State Injection:**

The `input` parameter controls which state variables are available in `elitea_state`:

- **Default behavior (no `input` or `input: ["messages"]`)**: ALL state variables are injected into `elitea_state`
- **Specific variables (`input: ["var1", "var2"]`)**: ONLY the specified variables are injected into `elitea_state`
- The `messages` variable is never injected into `elitea_state` (always excluded)
...
- id: "code_with_all_vars"
  ...
  code:
    ...
    value: |
      ...
      user_id = elitea_state.get('user_id')
      settings = elitea_state.get('settings')
      score = elitea_state.get('score')
...
- id: "code_with_filtered_vars"
  ...
  code:
    type: fixed
    value: |
      # Only user_id and score are available in elitea_state
      user_id = elitea_state.get('user_id')  # ✓ Available
      score = elitea_state.get('score')      # ✓ Available
      settings = elitea_state.get('settings')  # ✗ Not available (returns None)
      {"result": f"User {user_id} with score {score}"}
...
previous_result = elitea_state.get('previous_calculation', 0)
...
When an EliteA client is available, it's automatically injected as `elitea_client` for API interactions:

```python
# Use the EliteA client for API calls (list available artifacts from bucket `test`)
elitea_client.list_artifacts(bucket_name='test')
```
...
- id: "process_data"
  ...
  code: 
    ...
    value: |
        ...
        raw_data = elitea_state.get('raw_data', [])
...
- id: "calculate_metrics"
  ...
  code: 
    ...
    value: |
        ...
        values = elitea_state.get('values', [])
...
- id: "external_api_call"
  ...
  code: 
    ...
    value: |
        ...
        query = elitea_state.get('search_query', '')
...
- id: "business_logic"
  ...
  code: 
    type: fixed
    value: |
        user_age = elitea_state.get('user_age', 0)
        user_type = elitea_state.get('user_type', 'guest')
  ...
  - id: "process_data"
    ...
    code: 
      ...
      value: |
          ...
          data = elitea_state.get('extracted_data', [])
