...
Enhanced the EliteA CLI agent support with four major improvements based on user feedback:
...
# Override with flag
elitea-cli agent list --local --directory custom/path
```
...
### Example Files
5. **`elitea-sdk/examples/agent-with-env-vars.agent.md`**
   - Complete example agent using all features
   - Demonstrates env vars in prompt
   - Shows inline toolkit configs

6. **`elitea-sdk/examples/env-example.env`**
   - Template `.env` file
   - Documents all common variables

7. **`elitea-sdk/examples/jira-config-with-env.json`**
...
# 1. Create test .env
cat > .env <<'EOF'
DEPLOYMENT_URL=https://test.elitea.ai
PROJECT_ID=123
API_KEY=test_key
...
elitea-cli agent show elitea-sdk/examples/agent-with-env-vars.agent.md

# 3. Test agents directory config
elitea-cli agent list --local

# 4. Test toolkit auto-add (with actual agent)
elitea-cli agent chat platform-agent --toolkit-config jira-config.json
```
...
- `elitea-sdk/examples/agent-with-env-vars.agent.md` - Full-featured agent
- `elitea-sdk/examples/env-example.env` - Environment variables template
- `elitea-sdk/examples/jira-config-with-env.json` - Toolkit config with env vars
- `elitea-sdk/docs/cli/custom-agent-example.md` - Complete workflow guide
- `elitea-sdk/docs/cli/AGENT_CLI_GUIDE.md` - Full documentation
...
2. **Create `.env` file** with your secrets
   ```bash
   cp elitea-sdk/examples/env-example.env .env
...
5. **Simplify your commands**
   ```bash
   # Before
   elitea-cli agent chat agent --toolkit-config a.json --toolkit-config b.json
   
   # After
   elitea-cli agent chat agent  # configs in agent file
   ```
