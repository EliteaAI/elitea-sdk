...
```bash
# Basic usage
elitea inventory ingest --dir ./my-project -g ./graph.json --preset python

# Combine preset with custom patterns
elitea inventory ingest --dir ./src -g ./graph.json -p typescript -w "*.json" -x "*backup*"
```
...
```python
from elitea_sdk.community.inventory import combine_presets
...
1. **Use the base preset and add patterns:**
   ```bash
   elitea inventory ingest --dir ./src -g ./graph.json \
     --preset python \
     -w "*.yaml" \
     -x "*_backup*"
   ```
...
2. **Create a preset in your project** (for programmatic use):
   ```python
   from elitea_sdk.community.inventory import IngestionPipeline
...
4. **Check stats** - Run `elitea inventory stats` to verify ingestion results
